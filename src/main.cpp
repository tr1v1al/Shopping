...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    int cols = M.columns();
    string * collabels = nullptr;
    string * rowlabels = nullptr;
    rowlabels = new string [1];
    rowlabels[0] = "pr";
    collabels = new string [cols];
    
    //for (int i=0; i<cols; i++)
    //    collabels[i] = M.labelAtCol(i);
    
    int numcols = M.getColLabels(collabels);
    
    Matrix pr(rowlabels,1,collabels,cols,1.0/cols);
    Matrix aux(pr);
    ...
    while (count < max_iter && !stable) {
        pr = aux * M;
        val = 0.0;
        for (int i=0; i<cols; i++) 
            val += abs(pr(0,i) - aux(0,i));
        if (val < threshold)
            stable = true;
        aux = pr;
        if (count < n)
            cout << "PageRank at iteration " << count << endl << pr << endl;
        count++;
    }
...
Matrix computePageRank(Matrix & M, int max_iter, double threshold, int n = 10) {
    ...
    cout << "After the teletransportation: " << endl;
    cout << M << endl;
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    for (int i = 0; i < idx.size(); i++)
        extrEvSet.add(evs.at(idx.at(i).getPos()));
    int num_ses = 0;
    string uniqueSessions[MAXBRANDS];
    findUnique(extrEvSet,"Session",uniqueSessions,num_ses);
    
    for (int l=0; l<num_ses; l++) {
        session = uniqueSessions[l];
...
int main(int argc, char** argv) {
    ...
    //valgrind --leak-check=full --track-origins=yes "${OUTPUT_PATH}" -input ./data/ECommerce162.dataset -K 5 -type cart -events 10000 -display Framed
